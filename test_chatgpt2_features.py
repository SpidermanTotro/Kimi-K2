#!/usr/bin/env python3
"""
Test suite for THE FORGE AI - ChatGPT 2.0 Features
Tests memory system, skills engine, multimodal, collaboration, plugins, ethics
"""

import os
import sys
import tempfile
import unittest
from pathlib import Path

# Add parent directory to path
sys.path.insert(0, str(Path(__file__).parent))


class TestForgeMemory(unittest.TestCase):
    """Test cases for the memory system"""
    
    def setUp(self):
        """Set up test fixtures"""
        from forge_memory import ForgeMemorySystem
        # Use a temporary database for testing
        self.temp_dir = tempfile.mkdtemp()
        self.db_path = os.path.join(self.temp_dir, "test_memory.db")
        self.memory = ForgeMemorySystem(self.db_path)
    
    def tearDown(self):
        """Clean up after tests"""
        if os.path.exists(self.db_path):
            os.remove(self.db_path)
    
    def test_store_and_recall_memory(self):
        """Test storing and recalling a memory"""
        content = "Python is a great programming language"
        memory_id = self.memory.store_memory(
            content=content,
            memory_type="fact",
            importance=0.8
        )
        
        self.assertIsNotNone(memory_id)
        self.assertTrue(len(memory_id) > 0)
        
        # Recall the memory
        recalled = self.memory.recall_memory(memory_id)
        self.assertIsNotNone(recalled)
        self.assertEqual(recalled.content, content)
        self.assertEqual(recalled.memory_type, "fact")
        self.assertEqual(recalled.importance, 0.8)
    
    def test_hierarchical_memory_levels(self):
        """Test hierarchical memory storage"""
        # Store memories at different levels
        working_id = self.memory.store_memory(
            content="Current task context",
            memory_type="context",
            memory_level="working"
        )
        
        episodic_id = self.memory.store_memory(
            content="Recent conversation topic",
            memory_type="conversation",
            memory_level="episodic"
        )
        
        semantic_id = self.memory.store_memory(
            content="User prefers Python",
            memory_type="preference",
            memory_level="semantic"
        )
        
        # Verify levels
        working_mem = self.memory.recall_memory(working_id)
        self.assertEqual(working_mem.memory_level, "working")
        
        episodic_mem = self.memory.recall_memory(episodic_id)
        self.assertEqual(episodic_mem.memory_level, "episodic")
        
        semantic_mem = self.memory.recall_memory(semantic_id)
        self.assertEqual(semantic_mem.memory_level, "semantic")
    
    def test_search_memories(self):
        """Test searching memories"""
        self.memory.store_memory(
            content="I love Python programming",
            memory_type="preference",
            importance=0.7
        )
        self.memory.store_memory(
            content="JavaScript is useful for web development",
            memory_type="fact",
            importance=0.6
        )
        
        results = self.memory.search_memories("Python")
        self.assertTrue(len(results) > 0)
        self.assertTrue(any("Python" in r.content for r in results))
    
    def test_conversation_storage(self):
        """Test storing and retrieving conversations"""
        session_id = "test_session_123"
        
        self.memory.store_conversation_turn(
            session_id=session_id,
            role="user",
            content="Hello, how are you?"
        )
        
        self.memory.store_conversation_turn(
            session_id=session_id,
            role="assistant",
            content="I'm doing great, thank you!"
        )
        
        history = self.memory.get_conversation_history(session_id)
        self.assertEqual(len(history), 2)
        self.assertEqual(history[0].role, "user")
        self.assertEqual(history[1].role, "assistant")
    
    def test_user_profile(self):
        """Test user profile creation and update"""
        user_id = "test_user_456"
        
        profile = self.memory.get_or_create_profile(user_id)
        self.assertEqual(profile.user_id, user_id)
        
        self.memory.update_profile(user_id, {
            "preferences": {"language": "python"},
            "topics_of_interest": ["programming", "ai"]
        })
        
        updated = self.memory.get_or_create_profile(user_id)
        self.assertEqual(updated.preferences.get("language"), "python")
        self.assertIn("programming", updated.topics_of_interest)
    
    def test_memory_stats(self):
        """Test getting memory statistics"""
        self.memory.store_memory(content="Test memory", memory_type="test")
        
        stats = self.memory.get_memory_stats()
        
        self.assertIn("total_memories", stats)
        self.assertIn("cached_memories", stats)
        self.assertIn("memories_by_level", stats)
        self.assertTrue(stats["total_memories"] >= 1)


class TestForgeSkills(unittest.TestCase):
    """Test cases for the skills engine"""
    
    def setUp(self):
        """Set up test fixtures"""
        from forge_skills import AdvancedSkillsEngine
        self.skills = AdvancedSkillsEngine()
    
    def test_chain_of_thought(self):
        """Test chain-of-thought reasoning"""
        result = self.skills.reasoning.chain_of_thought(
            "How can I improve my Python code performance?"
        )
        
        self.assertTrue(result.success)
        self.assertIsNotNone(result.output)
        self.assertEqual(result.skill_name, "chain_of_thought")
    
    def test_code_analysis(self):
        """Test code analysis"""
        sample_code = '''
def calculate(x):
    if x > 0:
        return x * 2
    return 0
'''
        result = self.skills.code_skills.analyze_code(sample_code, "python")
        
        self.assertTrue(result.success)
        self.assertIn("quality_score", result.output)
        self.assertIn("complexity", result.output)
    
    def test_code_generation(self):
        """Test code generation"""
        result = self.skills.code_skills.generate_code(
            "Create a function to process data",
            language="python"
        )
        
        self.assertTrue(result.success)
        self.assertIn("def", result.output)
    
    def test_writing_analysis(self):
        """Test writing analysis"""
        sample_text = """
        The implementation of artificial intelligence has revolutionized 
        how we approach software development. Furthermore, the integration
        of machine learning enables systems to improve over time.
        """
        
        result = self.skills.writing_skills.analyze_writing(sample_text)
        
        self.assertTrue(result.success)
        self.assertIn("word_count", result.output)
        self.assertIn("style", result.output)
        self.assertIn("readability", result.output)
    
    def test_intent_detection(self):
        """Test intent detection"""
        messages = [
            ("Please help me with this", "request"),
            ("What is Python?", "question"),
            ("Create a function", "command"),
        ]
        
        for message, expected_intent in messages:
            result = self.skills.personalization.detect_user_intent(message)
            self.assertTrue(result.success)
            self.assertEqual(result.output["primary_intent"], expected_intent)
    
    def test_available_skills(self):
        """Test getting available skills"""
        skills = self.skills.get_available_skills()
        
        self.assertIn("reasoning", skills)
        self.assertIn("coding", skills)
        self.assertIn("writing", skills)
        self.assertIn("personalization", skills)


class TestForgeMultimodal(unittest.TestCase):
    """Test cases for multimodal support"""
    
    def setUp(self):
        """Set up test fixtures"""
        from forge_multimodal import MultimodalProcessor, ContentDetector, ImageHandler
        self.processor = MultimodalProcessor()
        self.detector = ContentDetector
        self.image_handler = ImageHandler
    
    def test_content_type_detection_from_path(self):
        """Test content type detection from file path"""
        from forge_multimodal import ContentType
        
        test_cases = [
            ("image.png", ContentType.IMAGE),
            ("audio.mp3", ContentType.AUDIO),
            ("document.pdf", ContentType.DOCUMENT),
            ("script.py", ContentType.CODE),
        ]
        
        for path, expected_type in test_cases:
            detected = self.detector.detect_from_path(path)
            self.assertEqual(detected, expected_type)
    
    def test_supported_formats(self):
        """Test getting supported formats"""
        formats = self.processor.get_supported_formats()
        
        self.assertIn("image", formats)
        self.assertIn("audio", formats)
        self.assertIn("video", formats)
        self.assertIn("document", formats)
    
    def test_image_analysis(self):
        """Test basic image analysis"""
        # Create minimal PNG data
        sample_png = bytes([
            0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A,
            0x00, 0x00, 0x00, 0x0D,
            0x49, 0x48, 0x44, 0x52,
            0x00, 0x00, 0x00, 0x64,  # width: 100
            0x00, 0x00, 0x00, 0x64,  # height: 100
            0x08, 0x02,
            0x00, 0x00, 0x00,
        ])
        
        analysis = self.image_handler.analyze_image(sample_png)
        
        self.assertEqual(analysis.format, "png")
        self.assertEqual(analysis.width, 100)
        self.assertEqual(analysis.height, 100)


class TestForgeCollaboration(unittest.TestCase):
    """Test cases for collaboration features"""
    
    def setUp(self):
        """Set up test fixtures"""
        from forge_collaboration import CollaborationSystem, Role
        self.temp_dir = tempfile.mkdtemp()
        self.db_path = os.path.join(self.temp_dir, "test_collab.db")
        self.collab = CollaborationSystem(self.db_path)
        self.Role = Role
    
    def tearDown(self):
        """Clean up after tests"""
        if os.path.exists(self.db_path):
            os.remove(self.db_path)
    
    def test_create_workspace(self):
        """Test workspace creation"""
        workspace = self.collab.create_workspace(
            name="Test Workspace",
            description="A test workspace",
            created_by="alice"
        )
        
        self.assertIsNotNone(workspace)
        self.assertEqual(workspace.name, "Test Workspace")
        self.assertEqual(workspace.created_by, "alice")
        self.assertTrue(len(workspace.members) >= 1)
    
    def test_add_member(self):
        """Test adding members to workspace"""
        workspace = self.collab.create_workspace(
            name="Team Workspace",
            description="Team collaboration",
            created_by="alice"
        )
        
        result = self.collab.add_member(
            workspace.workspace_id,
            "bob",
            self.Role.EDITOR,
            "alice"
        )
        
        self.assertTrue(result)
        
        # Verify member was added
        ws = self.collab.get_workspace(workspace.workspace_id)
        member_ids = [m.user_id for m in ws.members]
        self.assertIn("bob", member_ids)
    
    def test_shared_memory(self):
        """Test shared memory operations"""
        workspace = self.collab.create_workspace(
            name="Memory Workspace",
            description="Shared memory test",
            created_by="alice"
        )
        
        memory_id = self.collab.add_shared_memory(
            workspace.workspace_id,
            "Shared knowledge item",
            "fact",
            "alice"
        )
        
        self.assertIsNotNone(memory_id)
        
        # Retrieve shared memories
        memories = self.collab.get_shared_memories(workspace.workspace_id, "alice")
        self.assertTrue(len(memories) >= 1)


class TestForgePlugins(unittest.TestCase):
    """Test cases for plugin system"""
    
    def setUp(self):
        """Set up test fixtures"""
        from forge_plugins import PluginManager, LoggingPlugin, MetricsPlugin
        self.manager = PluginManager()
        self.LoggingPlugin = LoggingPlugin
        self.MetricsPlugin = MetricsPlugin
    
    def test_builtin_plugins_loaded(self):
        """Test that built-in plugins are loaded"""
        plugins = self.manager.list_plugins()
        
        self.assertTrue(len(plugins) > 0)
        plugin_names = [p.get("metadata", {}).get("name") for p in plugins]
        self.assertIn("LoggingPlugin", plugin_names)
        self.assertIn("MetricsPlugin", plugin_names)
    
    def test_plugin_enable_disable(self):
        """Test enabling and disabling plugins"""
        self.manager.enable_all()
        enabled = self.manager.registry.list_enabled_plugins()
        self.assertTrue(len(enabled) > 0)
        
        self.manager.disable_all()
        enabled = self.manager.registry.list_enabled_plugins()
        self.assertEqual(len(enabled), 0)
    
    def test_hook_execution(self):
        """Test hook execution through plugins"""
        self.manager.enable_all()
        
        test_input = "Hello, test message"
        processed = self.manager.process_input(test_input)
        
        # Logging plugin should pass through unchanged
        self.assertEqual(processed, test_input)


class TestForgeEthics(unittest.TestCase):
    """Test cases for ethics and safety"""
    
    def setUp(self):
        """Set up test fixtures"""
        from forge_ethics import EthicsGuard, ContentModerator, BiasDetector, PrivacyProtector
        self.temp_dir = tempfile.mkdtemp()
        self.db_path = os.path.join(self.temp_dir, "test_ethics.db")
        self.guard = EthicsGuard(db_path=self.db_path)
        self.moderator = ContentModerator()
        self.bias_detector = BiasDetector()
        self.privacy = PrivacyProtector()
    
    def tearDown(self):
        """Clean up after tests"""
        if os.path.exists(self.db_path):
            os.remove(self.db_path)
    
    def test_safe_content_analysis(self):
        """Test analysis of safe content"""
        result = self.guard.analyze_input("Please help me write a Python function.")
        
        self.assertTrue(result["is_safe"])
        self.assertEqual(result["action"], "allow")
    
    def test_privacy_detection(self):
        """Test PII detection"""
        content = "Contact me at john.doe@email.com"
        analysis = self.privacy.analyze(content)
        
        self.assertTrue(analysis.has_pii)
        self.assertIn("email", analysis.pii_types)
        self.assertIsNotNone(analysis.redacted_content)
    
    def test_pii_redaction(self):
        """Test PII redaction"""
        content = "My SSN is 123-45-6789 and email is test@example.com"
        redacted = self.privacy.redact_all(content)
        
        self.assertNotIn("123-45-6789", redacted)
        self.assertNotIn("test@example.com", redacted)
        self.assertIn("[REDACTED-SSN]", redacted)
        self.assertIn("[REDACTED-EMAIL]", redacted)
    
    def test_bias_detection(self):
        """Test bias detection"""
        biased_content = "All young people can't understand technology."
        analysis = self.bias_detector.analyze(biased_content)
        
        self.assertTrue(analysis.has_bias)
        self.assertTrue(len(analysis.bias_types) > 0)
    
    def test_guidelines(self):
        """Test ethical guidelines retrieval"""
        guidelines = self.guard.get_guidelines()
        
        self.assertIn("content_policy", guidelines)
        self.assertIn("privacy_policy", guidelines)
        self.assertIn("bias_policy", guidelines)


class TestKimiForgeUnified(unittest.TestCase):
    """Test cases for the unified system"""
    
    def setUp(self):
        """Set up test fixtures"""
        from kimi_forge_unified import KimiForgeUnified
        self.temp_dir = tempfile.mkdtemp()
        self.db_path = os.path.join(self.temp_dir, "test_unified.db")
        self.system = KimiForgeUnified(db_path=self.db_path)
    
    def tearDown(self):
        """Clean up after tests"""
        for suffix in ["_memory.db", "_collaboration.db", "_ethics.db"]:
            path = self.db_path.replace(".db", suffix)
            if os.path.exists(path):
                os.remove(path)
        if os.path.exists(self.db_path):
            os.remove(self.db_path)
    
    def test_basic_process(self):
        """Test basic processing"""
        response = self.system.process("Write a Python function")
        
        self.assertIsNotNone(response)
        self.assertTrue(len(response) > 0)
    
    def test_memory_enhanced_process(self):
        """Test memory-enhanced processing"""
        result = self.system.process_with_memory(
            "Help me with Python programming",
            user_id="test_user",
            session_id="test_session"
        )
        
        self.assertIn("response", result)
        self.assertIn("session_id", result)
        self.assertTrue(len(result["response"]) > 0)
    
    def test_remember_fact(self):
        """Test fact storage"""
        if self.system.memory:
            fact_id = self.system.remember_fact(
                "User likes Python",
                importance=0.8,
                tags=["preference"]
            )
            
            self.assertIsNotNone(fact_id)
            self.assertTrue(len(fact_id) > 0)
    
    def test_get_stats(self):
        """Test getting system statistics"""
        stats = self.system.get_stats()
        
        self.assertIn("kimi_k2_model", stats)
        self.assertIn("forge_tools_available", stats)
        self.assertIn("chatgpt_2_features", stats)
        
        features = stats["chatgpt_2_features"]
        self.assertIn("memory_system", features)
        self.assertIn("skills_engine", features)
        self.assertIn("multimodal_support", features)
        self.assertIn("collaboration", features)
        self.assertIn("plugin_system", features)
        self.assertIn("ethics_guard", features)
    
    def test_user_profile(self):
        """Test user profile retrieval"""
        if self.system.memory:
            profile = self.system.get_user_profile("new_user")
            
            self.assertIsNotNone(profile)
            self.assertEqual(profile["user_id"], "new_user")


def run_tests():
    """Run all tests"""
    loader = unittest.TestLoader()
    suite = unittest.TestSuite()
    
    # Add test classes
    suite.addTests(loader.loadTestsFromTestCase(TestForgeMemory))
    suite.addTests(loader.loadTestsFromTestCase(TestForgeSkills))
    suite.addTests(loader.loadTestsFromTestCase(TestForgeMultimodal))
    suite.addTests(loader.loadTestsFromTestCase(TestForgeCollaboration))
    suite.addTests(loader.loadTestsFromTestCase(TestForgePlugins))
    suite.addTests(loader.loadTestsFromTestCase(TestForgeEthics))
    suite.addTests(loader.loadTestsFromTestCase(TestKimiForgeUnified))
    
    # Run tests
    runner = unittest.TextTestRunner(verbosity=2)
    result = runner.run(suite)
    
    return result.wasSuccessful()


if __name__ == "__main__":
    print("=" * 70)
    print("🧪 THE FORGE AI - ChatGPT 2.0 Features Test Suite")
    print("=" * 70)
    print()
    
    success = run_tests()
    
    print()
    print("=" * 70)
    if success:
        print("✅ All tests passed!")
    else:
        print("❌ Some tests failed!")
    print("=" * 70)
    
    sys.exit(0 if success else 1)
