...
- **Hierarchical memory management** (NEW)
...
class MemoryLevel(Enum):
    """Hierarchical memory levels for efficient storage and retrieval"""
    WORKING = "working"       # Short-term, current session
    EPISODIC = "episodic"     # Recent sessions, conversation context
    SEMANTIC = "semantic"     # Long-term facts and knowledge
    PROCEDURAL = "procedural" # Skills and procedures learned
...
class MemoryEntry:
    ...
    memory_level: str = "semantic"  # Hierarchical level
...
class ForgeMemorySystem:
    ...
    def __init__(self, db_path: str = "forge_memory.db", cache_size: int = 1000):
        ...
        self.working_memory: Dict[str, MemoryEntry] = {}  # Short-term
        self.episodic_memory: Dict[str, MemoryEntry] = {}  # Recent sessions
    ...
    def _init_database(self):
        ...
        # Memories table with hierarchical level support
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS memories (
                ...
                tags TEXT DEFAULT '[]',
                source TEXT DEFAULT '',
                memory_level TEXT DEFAULT 'semantic'
            )
        ''')
        ...
        # Memory associations table (for hierarchical connections)
        cursor.execute('''
    ...
    def _load_cache(self):
        ...
        for row in cursor.fetchall():
            memory = MemoryEntry(
                ...
                source=row[9] or "",
                memory_level=row[10] if len(row) > 10 else "semantic"
            )
    ...
    def store_memory(
        ...
        Args:
            ...
            memory_level: Hierarchical level (working, episodic, semantic, procedural)
        ...
        cursor.execute('''
            ...
            (id, content, memory_type, importance, created_at, last_accessed, 
             access_count, context, tags, source, memory_level)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            memory.id,
            memory.content,
        ...
        # Add to appropriate cache based on level
        ...
        logger.debug(f"💾 Stored memory: {memory_id[:8]}... [{memory_type}] [{memory_level}]")
        return memory_id
    ...
    def search_memories(
        ...
        Args:
            ...
            memory_level: Filter by hierarchical level
    ...
    def get_memories_by_level(
        self,
        memory_level: str,
        limit: int = 50
    ) -> List[MemoryEntry]:
        """Get memories filtered by hierarchical level"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
    ...
    def promote_memory(self, memory_id: str, new_level: str) -> bool:
        """
        Promote a memory to a higher hierarchical level
        
        Args:
            memory_id: The memory to promote
            new_level: The new hierarchical level
        ...
        # Update cache
        if memory_id in self.memory_cache:
            self.memory_cache[memory_id].memory_level = new_level
    ...
    def consolidate_memories(self) -> int:
        ...
        This is part of the hierarchical memory management system.
        Memories that are frequently accessed or important get promoted.
        ...
        for mem_id, memory in list(self.working_memory.items()):
            if memory.access_count >= 3 or memory.importance >= 0.7:
                self.promote_memory(mem_id, "episodic")
                self.episodic_memory[mem_id] = memory
                del self.working_memory[mem_id]
                consolidated += 1
        
        # Promote important episodic memories to semantic
        for mem_id, memory in list(self.episodic_memory.items()):
            if memory.access_count >= 5 or memory.importance >= 0.8:
                self.promote_memory(mem_id, "semantic")
    ...
    def create_association(
        ...
        Args:
            ...
            associated_memory_id: Second memory ID
            association_type: Type of association (e.g., 'related', 'causes', 'follows')
        ...
        try:
            cursor.execute('''
                ...
                (memory_id, associated_memory_id, association_type, strength, created_at)
                VALUES (?, ?, ?, ?, ?)
            ''', (memory_id, associated_memory_id, association_type, strength, now))
    ...
    def get_associated_memories(
        ...
        Returns:
            List of tuples (associated_memory_id, association_type, strength)
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        if association_type:
            cursor.execute('''
                SELECT associated_memory_id, association_type, strength
                FROM memory_associations
                WHERE memory_id = ? AND association_type = ?
            ''', (memory_id, association_type))
        else:
            cursor.execute('''
                SELECT associated_memory_id, association_type, strength
                FROM memory_associations
...
def main():
    ...
    # Store some memories with hierarchical levels
    print("💾 Storing memories with hierarchical levels...")
    ...
    memory.store_memory(
        content="Currently discussing Python optimization",
        memory_type="context",
        importance=0.5,
    ...
    for mem in results:
        print(f"  - [{mem.memory_type}] [{mem.memory_level}] {mem.content[:50]}...")
    
    # Get memories by level
    print("\n📊 Memories by hierarchical level:")
    for level in ["working", "episodic", "semantic"]:
        mems = memory.get_memories_by_level(level, limit=5)
        print(f"  {level}: {len(mems)} memories")
